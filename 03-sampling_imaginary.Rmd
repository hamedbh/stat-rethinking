...
ppd <- samples01 %>% 
  mutate(
    w_PPD = rbinom(n = nrow(.), size = 9, prob = p_sample), 
    w_MAP = rbinom(n = nrow(.), size = 9, prob = 0.67)  
  )
ppd
```
...
hw_m_ppd <- hw_m_samples %>% 
  mutate(w_PPD = rbinom(n = nrow(.), size = 15, prob = p))
...
```{r}
hw_m_samples %>% 
  mutate(draws = rbinom(nrow(.), size = 9, prob = p)) %>% 
...
It seems that 3,200 tosses is rather more than is required, as we get the desired width in the posterior 99% interval somewhere between 2,200 and 2,300. 
