...
waffle_dag %>% 
    ...
    ggdag() + 
    theme_void() + 
    labs(caption = paste0("S: Southern US; ", 
...
Analysis above implied two possible DAGs for assessing the total causal influence of Waffle Houses ($W$) on divorce rate ($D$), with the simplest being to condition only on being a Southern state ($S$). That would imply a model:

$$
\begin{eqnarray}
D_{i} & \sim & \mathcal{N}(\mu_i, \sigma) \\
\mu_i & = & \alpha + \beta_W W_i + \beta_S S_i \\
\alpha_i & \sim & \mathcal{N}(0, 1) \\
\beta_W & \sim & \mathcal{N}(0, 2) \\
\beta_S & \sim & \mathcal{N}(0, 2) \\
\sigma & \sim & \text{Exponential}(1)
\end{eqnarray}
$$
...
b6_13 <- brm(
    ...
    chains = 4, 
    cores = 4, 
    file = "Stan/ch_06/b6_13"
)
posterior_summary(b6_13) %>% 
    round(digits = 2)
