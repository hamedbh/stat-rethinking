...
\text{AIC} &= -2\text{lppd} + 2p \\
           &= -2(\text{lppd} - p) \\
WAIC(y, \Theta) &= -2(\text{lppd} - \sum_i \text{var}_{\theta} \log \operatorname{P}(y_i|\theta))
\end{align}
$$
...
To make the two the same we would need $p \approx \sum_i \text{var}_{\theta} \log \operatorname{P}(y_i|\theta)$. 
