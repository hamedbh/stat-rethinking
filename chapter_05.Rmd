...
The evidence is now fairly strong that area has a positive association with weight, and that group size has a negative one. These were probably masking each other in the simple linear regression. Can test this by looking at the correlation between groupsize and area.
...
```{r}
# first fit multiple regression with avgfood and groupsize
b5_19a <- brm(
    weight_norm ~ 1 + avgfood + groupsize, 
    data = d, 
    prior = c(prior(normal(0, 1), class = "Intercept"), 
              prior(normal(0, 0.5), class = "b"), 
              prior(exponential(1), class = "sigma")), 
    ...
    warmup = 1000, 
    cores = 4, 
    file = "./Stan/ch_05/b5_19a"
)
# now add area to the last model
b5_19b <- brm(
    weight_norm ~ 1 + avgfood + groupsize + area, 
    data = d, 
    prior = c(prior(normal(0, 1), class = "Intercept"), 
              prior(normal(0, 0.5), class = "b"), 
              prior(exponential(1), class = "sigma")), 
    ...
    warmup = 1000, 
    cores = 4, 
    file = "./Stan/ch_05/b5_19b"
)
# inspect both summaries
summary(b5_19a)
summary(b5_19b)
```
...
To answer the question of which predictor is better, start by inspecting some plots. First the dotplots for the parameter estimates on model `b5_18` (which includes `area`) and `b5_19a` (`avgfood`).
...
The CI for the coefficient of `avgfood` is very wide, whereas that for `area` is fairly small. What about the model with all of the predictors?
...
The coefficient estimate for `avgfood` is not dissimilar to the last plot, but that for area is now wider. Perhaps the two predictors are behaving similarly to the left and right leg when predicting height?
...
The two predictors are strongly correlated and both have some sort of positive relationship with the outcome. This suggests that one of them is probably redundant, and given the wide parameter estimate CI for `avgfood` I'd be inclined to keep `area` instead. 
