...
logistic_lines <- tibble(x = seq(from = -1, to = 1, by = .25)) %>% 
  mutate(log_measurement  = 4 * x) %>% 
  mutate(natural = inv_logit(log_odds))
logistic_example <- tibble(x = seq(-1, 1, length.out = 101)) %>% 
  mutate(log_measurement = 2.1 * x) %>% 
  mutate(natural = inv_logit(log_odds))
...
loglink_lines <- tibble(log_measurement = seq(from = -3, to = 3)) %>% 
  mutate(natural = exp(log_measurement))
...
(
  ...
  ggplot(loglink_example, aes(x, natural)) + 
    geom_line(colour = "steelblue", size = 1) + 
    geom_hline(
      aes(yintercept = natural), 
      data = loglink_lines %>% 
        filter(natural <= 10), 
      alpha = 0.2
    ) + 
