...
sim(m8h4c) %>% 
  ...
  inner_join(
    dnettle %>% 
      filter(
        country %in% c("Vanuatu", "Cuba", "Papua New Guinea", "Brazil")
      ) %>% 
      select(country, L), 
    by = "country"
  ) %>% 
...
```{r}
set.seed(1135)
contrasts_8h6 <- extract.samples(m8h6) %>% 
  `[`(c("aF", "aWUS", "aJUS")) %>% 
  reduce(cbind) %>% 
  as_tibble() %>% 
  transmute(
...
contrasts_8h6 %>% 
  ...
  mutate(split_loc = str_locate(.[["contrast"]], "_")[, 1]) %>% 
...
We can then use `link()` as RM suggests to get draws from the linear model. The plot is inspired by [Gregor Mathes'](https://gregor-mathes.netlify.app/2021/03/22/rethinking-chapter-8/#hard-practices) effort. 
