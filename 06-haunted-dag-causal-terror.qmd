...
We now set up the model as though we don't already know the data generating structure, but using some scientific knowledge and physical constraints. 
...
Now if we want to understand whether age is related to happiness we can build a model. The linear model part is: 
...
```{r}
coeftab(m6_11, m6_11b, m6_11c) |> 
  coeftab_plot()
```

We get similar inferences for the effect of parents in `m6_11` and `m6_11b`, but for everything else the results are markedly different across the models. 
...
```{r}
coeftab(m6_11, m6_11b, m6_11c, m6_12) |> 
  coeftab_plot()
```
...
```{r}
plot_DAG_for_grid <- function(d, title) {
  p <- d |>
    tidy_dagitty() |> 
    ggplot(aes(x = x, y = y, xend = xend, yend = yend)) + 
...
map2(
  dag_list,
  c("The Fork", "The Pipe", "The Collider", "The Descendent"),
  plot_DAG_for_grid
) |> 
  wrap_plots()
```
...
RM suggests that $C$ will be better in this particular case because it will tell us about the precision of our estimate of $X \to Y$. Maybe this will make more sense later in the book when we cover measurement? For now though we can try a simulation study to see whether it makes any difference.

```{r}
set.seed(1354)
d_two_roads <- tibble(A = rnorm(1000)) |> 
  mutate(
    U = rnorm(1000, mean = A),
    C = rnorm(1000, mean = A)
  ) |> 
  mutate(
    B = rnorm(1000, mean = U + C),
    X = rnorm(1000, mean = U)
  ) |> 
  mutate(Y = rnorm(1000, mean = X + C))
```

First condition on `C`.
...
The coefficient parameter estimates are still about right, but very slightly worse than before. However `sigma` is much higher now, so we could expect much more variance in our posterior predictions.
...
We've already been using median age marriage ($A$) and marriage rate ($M$) as predictors, but the usefulness of $M$ was close to zero once we know $A$, so let's drop that. We also have the indicator of a state being in the South, $S$. 
...
tibble(a = rnorm(N, sd = 0.2), bA = rnorm(N, sd = 0.5)) |> 
  ...
  geom_line(colour = "steelblue", alpha = 0.2) + 
...
This question extends the exercises from the book. Now we need to find total causal effect of area on food. 

```{r}
dag_fox |> 
  adjustmentSets(exposure = "area", outcome = "avgfood")
```

No need to condition, we can regress directly. 

$$
\begin{align*}
F &\sim \mathcal{N}(\mu_i, \sigma) \\
\mu_i &= \alpha +\beta_A A_i\\
\alpha &\sim \mathcal{N}(0, 0.2) \\
\beta_A &\sim \mathcal{N}(0, 0.5) \\
\sigma &\sim \text{Expo}(1)
...
This suggests a very strong association between area and avgfood: increasing area by 1 standard deviation increases food by 0.8-0.95 standard deviations. 

#### Q2. 

This is a repeat of question 6H4 from the book. Just as a reminder we can check the adjustment sets again. 

```{r}
dag_fox |> 
  adjustmentSets("avgfood", "weight")
```

No controls necessary, we just regress directly. This is model `m6h4`.
...
There is very little effect on weight of changing food. However the picture changes when we wish to isolate the direct causal effect. This was covered from a different angle in question 6H5 from the book: then we wanted the effect of groupsize on weight, for which we had to shut the backdoor path from avgfood (which was a fork). Now the focus is different but the approach is the same: regress weight jointly on groupsize (which is a pipe from avgfood to weight) and avgfood. This was model `m6h5`. 
...
We see that the direct causal effect of avgfood on weight seems positive, 0.19-0.76. 
...
We can check our adjustment set for $Y ~ X$. 

```{r}
adjustmentSets(stroke_dag, "X", "Y")
```

Both $A$ and $S$ are pipes in this DAG, with backdoor paths into $X$. So we need to include both in the regression, something like: 

$$
\begin{align*}
Y_i &\sim \mathcal{N}(\mu, \sigma) \\
\mu &= \alpha + \beta_A A + \beta_S S
\end{align*}
$$

The coefficient for $X$ in our model will be what we intend. However including $S$ in the model opens up the backdoor through $U$ (fork), so we cannot trust its coefficient. This same fork will bias our estimate for $A$ (which has a path running $ASUY$).

#### Q4. 

```{r}
sim_stroke_data <- function(N = 1000) {
  tibble(
  A = rnorm(N), 
  U = rnorm(N)
) |> 
  mutate(S = rnorm(N, A + U)) |> 
  mutate(X = rnorm(N, A + S)) |> 
  mutate(Y = rnorm(N, A + S + X + U))
}
sim_stroke_data()
```

If our model is working effectively then it should recover the true coefficient for X, i.e. 1. 

```{r}
model_stroke_XY <- function(N = 1000) {
  mod <- quap(
    alist(
      Y ~ dnorm(mu, sigma), 
      mu <- a + (bX * X) + (bA * A) + (bS * S), 
    ...
    data = sim_stroke_data(N = N)
  )
  summ <- precis(mod)
  tibble(
    N = N,
    mean = mod@coef["bX"], 
    lower = summ@.Data[[3]][summ@row.names == "bX"], 
    upper = summ@.Data[[4]][summ@row.names == "bX"]
  )
}
model_stroke_XY()
```

Now we can address the question of what sort of sample size we need through simulation. 

```{r}
stroke_repeats <- xfun::cache_rds({
  ...
  file = "stroke_repeats.rds"
)
```

Now we can check how close our estimates were, and how narrow the PI for the estimate. 

```{r}
stroke_repeats |> 
  mutate(err = abs(mean - 1), width = upper - lower) |> 
  pivot_longer(c(err, width), names_to = "measure") |> 
  reframe_mean_PI(c(N, measure)) |> 
  pivot_wider() |> 
  ggplot(aes(x = N)) + 
...
The error proportion has levelled off by 256 samples. The width of the PI at 256 samples is extremely small, and has collapsed almost entirely by the time we get to 512 samples. So it would seem that 256 samples should be quite sufficient to give accurate estimates. 
