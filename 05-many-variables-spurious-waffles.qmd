...
quap(
  ...
  {\(x) link(x, post = extract.prior(x), data = list(A = c(-2, 2)))}() |> 
  as_tibble(.name_repair = ~ str_c(c(-2, 2))) |> 
  rowid_to_column(".id") |> 
  pivot_longer(
    -.id, 
...
prior_mu_5_2 |> 
  ...
  ggplot(aes(x, value, group = .id)) + 
  geom_line(alpha = 0.3) + 
  labs(y = "Divorce rate (std)", x = "Marriage rate (std)") + 
  theme(panel.grid = element_blank())
```
...
fig_5_4_1 <- mu_5_4 |> 
  ggplot(aes(A, M)) + 
  geom_point(alpha = 0.4, colour = "steelblue") + 
  geom_segment(aes(xend = A, yend = mu), linewidth = 1/4) + 
  geom_line(aes(y = mu)) + 
  geom_text_repel(
...
fig_5_4_2 <- mu_5_4_b |> 
    ggplot(aes(M, A)) + 
    geom_point(alpha = 0.4, colour = "steelblue") + 
    geom_segment(aes(xend = M, yend = mu), linewidth = 1/4) + 
    geom_line(aes(y = mu)) + 
    geom_text_repel(
...
link(m5_3) |> 
  {
    \(x) {
      tibble(
      mean = colMeans(x), 
      lower = apply(x, 2, PI)[1, ], 
      upper = apply(x, 2, PI)[2, ]
    )
    }
  }() |> 
  bind_cols(d) |> 
...
sim_5_3_A <- post_5_3_A |> 
  ...
  {\(x) mutate(x, M = rnorm(nrow(x), mean = aM + (bAM * A), sd = sigma_M))}() |> 
  # Now that we have an M for each set of parameters we use that to generate
  # predictions for D
  {
    \(x) mutate(
      x, 
      D = rnorm(nrow(x), mean = a + (bA * A) + (bM * M), sd = sigma)
    )
  }()

# reframe_mean_PI() is my own function to keep the pipelines simpler for
# something I end up doing a lot
reframe_mean_PI

# We can now summarise the predictions, ready for plotting
summary_5_3_A <- sim_5_3_A |> 
  pivot_longer(c(M, D), names_to = "var") |> 
  reframe_mean_PI(c(A, var)) |> 
  pivot_wider(names_from = c(var, name))
...
post_5_3_A |> 
  ...
  {\(x) mutate(x, D = rnorm(nrow(x), mean = a + (bM * M), sd = sigma))}() |> 
  pivot_longer(c(D), names_to = "var") |> 
  reframe_mean_PI(c(M, var)) |> 
  pivot_wider(names_from = c(var, name)) |> 
  ggplot(aes(M)) + 
    geom_line(aes(y = D_mean)) + 
    geom_ribbon(aes(ymin = D_lower, ymax = D_upper), alpha = 0.3) + 
...
m_5_8 |> 
  extract.samples() |> 
  {\(x) list(sigma = x$sigma, a = x$a, diff_fm = x$a[, 1] - x$a[, 2])}() |> 
  precis(depth = 2)
```
...
```{r}
read_tsv(here::here("data/LDS.tsv"), col_types = cols(Loc = col_factor()))
```
...
post_hw_5h4 <- extract.samples(m_hw_5h4) |> 
  reduce(cbind) |> 
  as_tibble(.name_repair = ~ c("bA", "sigma", "a1", "a2")) |> 
...
post_hw_5h4 |> 
  ...
  reframe_mean_PI(variable) |> 
  pivot_wider(names_from = name) |> 
  ggplot(aes(y = fct_rev(variable), colour = fct_rev(variable))) + 
...
sim(m_hw_height, data = hw_new_heights) |> 
  ...
  pivot_longer(
    everything(), 
    names_to = "height", 
    names_transform = list(height = parse_number)
  ) |> 
  reframe_mean_PI(height) |> 
  pivot_wider()
```
...
tibble(
  ...
  expand(
    nesting(rowid, a, bA, sigma),
    A = seq(0, 12.5, by = 0.5)
  ) |> 
  {\(x) mutate(x, W = rnorm(nrow(x), mean = a + (bA * A), sd = sigma))}() |> 
  ggplot(aes(A, W)) + 
...
There's not much difference we can discern in the distributions like this. We can also plot how many girls are heavier at each age.

```{r}
post_weight_sex |> 
  group_by(age) |> 
  summarise(prop_girls_heavier = mean(weight < 0)) |> 
  ggplot(aes(age, prop_girls_heavier)) +
  stat_smooth(
    method = "loess",
    formula = y ~ x,
    colour = "grey50"
  ...
  labs(
    title = "Proportion of girls heavier at each age",
    subtitle = "Grey line and band show LOESS-smoothed values"
  )
```
...
extract.samples(m_oxboys) |> 
  as_tibble() |> 
  {\(x) mutate(x, x = exp(rnorm(nrow(x), mu, sigma)))}() |> 
  ggplot(aes(x)) +
  geom_density() +
  geom_vline(
    aes(xintercept = x), 
