...
```{r}
precis_cmdstan(m11_hwh4a, variables = c("a", "bG[1]", "bG[2]"))
```
...
m11_hwh4a |> 
  spread_draws(a, bG[i]) |> 
  ungroup() |> 
  transmute(.draw, name = c("f", "m")[i], a, value = bG) |> 
  pivot_wider(names_prefix = "bG") |> 
  transmute(
    gender_gap_abs = bGm - bGf,
    gender_gap_prob = inv_logit(a + bGm) - inv_logit(a + bGf)
  ) |> 
  pivot_longer(
    everything(),
    names_to = "grp",
...
```{r}
precis_cmdstan(m11_hwh4b, variables = c("a", "bG[1]", "bG[2]"))
```
Both gender coefficient estimates still negative but the intercept is quite different. Let's get the same plot as before with the contrasts.
...
The probability difference is much smaller, so in terms of success rate there is no longer a substantial gender gap.

The interpretation is that the total effect of gender is very small, because when it's mediated by discipline that offsets some of the direct effect. We can simply plot the success rates by discipline to get an idea of what's going on.
...
dnwo |> 
  ...
  labs(
    title = "Application success rate by subject",
    y = NULL,
    x = NULL
  )
```

In the four largest areas, which comprise `r dnwo |> count(D, wt = A, sort = TRUE) |> mutate(cum_prop = cumsum(n) / sum(n)) |> slice(4) |> pull(cum_prop) |> scales::label_percent()()` of the applications, three have substantial advantages for men.
