...
The original model has more effective parameters, because there really _are_ more parameters in that model.
...
```{r}
set.seed(1707)
island_pops <- sample(10) 
hw_9h5_pos <- rep(0L, 1e5)
current <- sample(10, 1)
for (i in seq_len(1e5)) {
  hw_9h5_pos[i] <- current
  proposal <- current + sample(c(-1L, 1L), size = 1)
  if (proposal < 1L) proposal <- 10L
  if (proposal > 10L) proposal <- 1L
...
```{r}
inner_join(
  tibble(island = 1:10, island_pops) %>% 
    transmute(island, prop = island_pops / sum(island_pops)), 
  tibble(island = hw_9h5_pos) %>% 
    count(island) %>% 
...
1,000 draws is enough to get a very good approximation to the analytical solution. 
...
```{r}
m6_9 <- stan_model(file = here::here("inst/Stan/m6_9.stan")) %>% 
  sampling(data = dmarriage_adults, seed = 1707)

m6_10 <- stan_model(file = here::here("inst/Stan/m6_10.stan")) %>% 
  sampling(data = dmarriage_adults, seed = 1711)
```

```{r}
compare(m6_9, m6_10, func = PSIS)
```
...
```{r}
precis(m6_9, depth = 2, pars = c("a[1]", "a[2]", "bA", "sigma"))
```
...
```{r}
m_hwk4_q3a <- stan_model(file = here::here("inst/Stan/m_hwk4_q3a.stan")) %>% 
  sampling(data = compose_data(cherry_full), seed = 1841)
```

```{r}
m_hwk4_q3b <- stan_model(file = here::here("inst/Stan/m_hwk4_q3b.stan")) %>% 
  sampling(data = compose_data(cherry_full), seed = 1843)
```
...
post_cherry <- spread_draws(m_hwk4_q3a, a, bT, sigma) %>% 
  mutate(doy = rnorm(nrow(.), a + (bT * new_temp), sigma)) %>% 
  mutate(
...
Given that our new data point was extreme it's not so surprising that we get some extreme predictions. We even get a few predictions below zero, i.e. the first blossom will occur in the previous year. 
...
tibble(
  ...
  mutate(mass = rnorm(nrow(.), age * bA, sigma)) %>% 
...
```{r}
hwk4_dino_lin <- stan_model(file = here::here("inst/Stan/hwk4_dino_lin.stan")) %>% 
  sampling(data = ddino, seed = 1914)
```

```{r}
hwk4_dino_VB <- stan_model(file = here::here("inst/Stan/hwk4_dino_VB.stan")) %>% 
  sampling(data = ddino, seed = 758)
```

```{r}
compare(hwk4_dino_lin, hwk4_dino_VB, func = PSIS)
```
...
```{r}
hwk4_dino_VB2 <- stan_model(
  file = here::here("inst/Stan/hwk4_dino_VB2.stan")
) %>% 
  sampling(data = ddino, seed = 758)
```
