...
(NB. the warning about the high values of $k$ is for model `m6_7`. The values are above 0.5 but below 0.7, so we ignore the warning and carry one for now.)
...
tibble(
  ...
  summarise(dSE = sqrt(nrow(.) * var(diff_waic)))
```
...
We get the same warning about high values of $k$, which is happening because of outliers (like Idaho). We can get the pointwise PSIS and plot things to make it clearer. 
...
Model selection is the process of selecting a model based on a score (e.g. PSIS). Model comparison is when we compares the scores across models. 
...
dlaffer <- Laffer %>% 
  as_tibble() %>% 
  mutate(
    tax_rate_2 = tax_rate^2, 
    tax_rate_3 = tax_rate^3
  ) %>% 
  mutate(across(.fns = standardize))
```
...
entropy_7h3 <- d_7h3 %>% 
  transmute(
    ...
    entropy = pmap_dbl(
      ., 
      ~ - sum(c(..2, ..3, ..4, ..5, ..6) * log2(c(..2, ..3, ..4, ..5, ..6)))
    )
  )
entropy_7h3
```
