...
\\
\sigma \sim \textrm{Exponential}(0, 1)
\\
...
The $\sim$ symbol indicates that these parameters are _stochastic_: we do not know them with certainty, so we use these techniques to estimate them.

Can also write this into Bayes' formula:

$$
P(p | w, n) = 
    \frac
        {\text{Binomial}(w | n, p) \text{Uniform}(p | 0, 1)}
...
Can put this model into Bayes' formula also:

$$
P(\mu, \sigma |h) = 
    \frac
        { \prod_i 
            \mathcal{N}(h_i | \mu, \sigma) 
            \mathcal{N}(\mu | 178, 20)
            \text{Uniform}(\sigma | 0, 50)}
        { \int \int \prod_i 
            \mathcal{N}(h_i | \mu, \sigma) 
            \mathcal{N}(\mu | 178, 20)
            \text{Uniform}(\sigma | 0, 50)
...
post$LL <- sapply(seq_len(nrow(post)), 
                  function(i) {
                      sum(
                          dnorm(
                              d2$height,
...
Now use a quadratic approximation, with the peak at the _Maximum A Posteriori_ (MAP), using `rethinking::quap()`.
...
```{r}
m4.1 <- quap(flist, data = d2, start = starts)
precis(m4.1)
```
...
From the `quap()` output can use `vcov()` to recover the variance-covariance matrix for the parameters.
...
To figure out whether the priors are reasonable we can simulate and see whether the results are plausible.
...
This is implying some strange things:

- many people with negative heights;
- many people taller than any human that has even lived;
- strong suggestion that weight and height are negatively correlated.

Can do better by changing the prior on $\beta$ to be lognormal:

$$
\beta \sim \text{Log-Normal}(0, 1) \implies \text{ln}(\beta) \sim \mathcal{N}(0, 1)
$$

This rules out a negative coefficient for the predictor in the linear model. The new priors are:
...
\sigma \sim \textrm{Uniform}(0, 50)
$$
Can simulate from this prior again and check the output.
...
A much more sensible set of priors. Now build the model with `quap()`.
...
$$
\frac{\prod_i \mathcal{N}(y_i|\mu, \sigma)\mathcal{N}(\mu|0, 10)\textrm{Uniform}(\sigma|0, 10)}{\int\int\prod_i \mathcal{N}(y_i|\mu, \sigma)\mathcal{N}(\mu|0, 10)\textrm{Uniform}(\sigma|0, 10) d{\mu} d\sigma}
$$
...
posterior_samples <- as.data.frame(
    mvrnorm(n = ntrials, 
            mu = coef(model), 
            Sigma = vcov(model)))
sample_mu <- sapply(weight.seq, function(weight) {
    posterior_samples$alpha + posterior_samples$beta * log(weight)
})
sample_mu_mean <- apply(sample_mu, 2, mean)
sample_mu_hpdi <- apply(sample_mu, 2, HPDI)
sample_heights <- sapply(weight.seq, function(weight) {
    rnorm(n = nrow(posterior_samples), 
          mean = posterior_samples$alpha + posterior_samples$beta * log(weight), 
          sd = posterior_samples$sigma)
})
sample_height_hpdi <- apply(sample_heights, 2, HPDI)
plot(height ~ weight, data = d, col = col.alpha(rangi2, 0.4))
lines(x = weight.seq, y = sample_mu_mean)
shade(object = sample_mu_hpdi, lim = weight.seq)
shade(object = sample_height_hpdi, lim = weight.seq)
```
