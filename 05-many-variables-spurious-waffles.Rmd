...
Let $x$ be the number of hours per week a person spends in the gym, $s$ the amount of protein shakes a person drinks each week, and $y$ the amount of of weight a person lifts in a week. Then we can set up our toy model and check the correlations. 
...
m_hw_5h2 <- quap(
  ...
  # Change M so that it is log() of the original marriage rate
  data = d
)
precis(m_hw_5h2)
```
...
States in the South are about 0.55 standard deviations above the mean, qhile the other states are 0.21 standard deviations _below_ the mean. That's quite a difference. What sort of DAG would make sense? 
