...
samples %>%
    ...
    stat_function(
        aes(seq(0, 1, length.out = sample_size), ..y..),
        fun = dbeta,
        n = sample_size,
        colour = "blue",
...
d %>%
    ggplot(aes(x = p)) +
    geom_line(aes(y = posterior)) +
    geom_ribbon(data = d %>% filter(p > q_10_and_90[1] &
                                        p < q_10_and_90[2]),
                aes(ymin = 0,
                    ymax = posterior)) +
    annotate(geom = "text",
...
map(list(samples,
    ...
    ~ .x %>%
        mode_qi(value))
# 4. Simply summarising the posterior samples
map(list(samples,
         samples2),
    ~ .x %>%
        summarise(
...
```{r}
point_estimates <- map_dfr(list(mean_qi,
                                median_qi,
                                mode_qi),
                           ~ .x(samples2)) %>%
    select(value,
...
(loss <- d2 %>%
        select(decision = p) %>%
        mutate(weighted_avg_loss = map_dbl(decision,
                                           make_loss)))
loss %>%
    top_n(-1,
          weighted_avg_loss)
