...
Analysis above implied two possible DAGs for assessing the total causal influence of Waffle Houses on divorce rate, with the simplest being to condition only on being a Southern state. That would imply a model:

```{r}
data("WaffleDivorce", package = "rethinking")
d <- WaffleDivorce %>% 
  as_tibble() %>% 
  mutate(W = WaffleHouses/Population)
head(d)
```
