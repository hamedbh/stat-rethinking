...
mu <- link(m4_3, data = tibble(weight = seq(25, 70))) |> 
  as_tibble(.name_repair = ~ paste0("w", seq(25, 70))) |> 
  rowid_to_column(".id") |> 
  pivot_longer(
    -.id,
    names_to = "weight",
    values_to = "height",
    names_transform = list(weight = parse_number)
  )
```
...
```{r}
get_89_PI <- function(x) tibble(x = PI(x), bound = c("lower", "upper"))
```

```{r}
mu_summary <- mu |> 
  reframe(get_89_PI(height), .by = weight) |> 
...
d <- Howell1 |> 
  as_tibble() |> 
  mutate(weight_s = as.double(scale(weight))) |> 
...
Can redo the last plot from above, using this new fit. 
...
mu2_summary <- link(
  ...
  as_tibble(.name_repair = ~ paste0("w", weight_seq)) |> 
  rowid_to_column(".id") |> 
  pivot_longer(
    -.id,
    names_to = "weight_s",
    values_to = "height",
    names_transform = list(weight_s = parse_number)
  ) |> 
  mutate(weight_s2 = weight_s^2) |>
  reframe(get_89_PI(height), .by = c(weight_s, weight_s2)) |> 
  pivot_wider(names_from = "bound", values_from = x) |> 
  mutate(
...
mu_summary_cubic <- link(
  ...
  mutate(weight_s2 = weight_s^2, weight_s3 = weight_s^3) |> 
  reframe(get_89_PI(height), .by = c(weight_s, weight_s2, weight_s3)) |> 
  pivot_wider(names_from = "bound", values_from = x) |> 
  mutate(
    height = coef(m4_6)[["a"]] + 
...
knot_list <- cherry_full |> 
  reframe(knots = quantile(year, probs = seq(0, 1, length.out = 15))) |> 
  rowid_to_column("id")
...
hw_plot_cherry <- function(k, w_sd) {
  knot_list <- cherry_full |> 
    reframe(knots = quantile(year, probs = seq(0, 1, length.out = k))) |> 
    rowid_to_column("id")
...
mu_summary_hw4h3 <- mu_hw4h3 |> 
  ...
  pivot_wider(names_from = "bound", values_from = x) |> 
  mutate(height = mean(post_hw4h3$a) + mean(post_hw4h3$b) * log(weight))
...
mu_summary_hw4h5 <- mu_hw4h5 |> 
  reframe(get_89_PI(doy), .by = temp) |> 
  pivot_wider(names_from = "bound", values_from = x) |> 
  mutate(doy = mean(post_hw4h5$a) + mean(post_hw4h5$b) * temp)
