...
McElreath also points out that interesting hypotheses are not of the form "all swans are black", but rather "black swans are rare". In that case falsification will not resolve the matter, instead a process of good measurement and sound probabilistic comparisons. 
...
Using tools such as cross-validation and information criteria helps to build models that make better predictions. As McElreath says: "fitting is easy, prediction is hard". This leads to discussion of overfitting and how it can undermine the predictive power of our models. 
...
McElreath wants this to be the default form of regression, and the book works to make that case. 
...
If we want to stay in the realm of pure prediction this may not matter so much, but otherwise we need to work on understanding these causes. McElreath will use [Directed Acyclic Graphs](https://en.wikipedia.org/wiki/Directed_acyclic_graph) (DAGs) as a tool for this. 
